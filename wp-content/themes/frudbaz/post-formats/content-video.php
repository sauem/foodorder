...
$frudbaz_video_url = function_exists( 'get_field' ) ? get_field( 'fromate_style' ) : NULL;

    if ( is_single() ):
    frudbaz_set_post_view(get_the_ID());
?>

    <div class="post_content mb-30 format_video">
        <?php if ( has_post_thumbnail() ): ?>
        <div class="video_wrap pos-rel mb-30">
            <div class="post_thumb pos-rel">
                <?php the_post_thumbnail( 'full', ['class' => 'img-responsive'] ); ?>
                <?php if(!empty( $frudbaz_video_url )) : ?>
                    <a class="popup-video video_icon" href="<?php print esc_url( $frudbaz_video_url );?>"><i class="fal fa-play"></i></a>
                <?php endif; ?>
            </div>
        </div>
        <?php endif; ?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'post_item format_video' );?>>
        <?php if ( has_post_thumbnail() ): ?>
