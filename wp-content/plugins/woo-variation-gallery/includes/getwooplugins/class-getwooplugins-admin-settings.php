	...
	class GetWooPlugins_Admin_Settings {
        ...
        */
        ...
        public static function popup_template_links($value){
            ...
            if( $value['is_pro'] ){
                return sprintf('<a data-template="%s" data-tip="%s" class="getwooplugins-help-tip pro-modal" href="#"></a>', esc_attr(self::normalize_id( $value['id'] )), esc_html__('Check how this feature works', 'woo-variation-gallery'));
            }
            
            if($value['help_preview'] ){
                return sprintf('<a data-template="%s" data-tip="%s" class="getwooplugins-help-tip help-modal" href="#"></a>', esc_attr(self::normalize_id( $value['id'] )), esc_html__('See how this feature works', 'woo-variation-gallery'));
            }
        }
...
<div class="input-wrapper <?php echo esc_attr($input_wrapper_class) ?>">
					...
					case 'single_select_page':
						...
						<tr class="<?php echo esc_attr($class) ?> single_select_page" <?php echo self::dependency_attribute($value); ?>>
							...
							<td class="forminp">
								<?php echo str_replace( ' id=', " data-placeholder='" . esc_attr__( 'Select a page&hellip;', 'woo-variation-gallery' ) . "' style='" . esc_attr($value['css']) . "' class='" . esc_attr($value['class']) . "' id=", wp_dropdown_pages( $args ) ); // WPCS: XSS ok. ?> <?php echo wp_kses_post($description); ?>
							</td>
						</tr>
						<?php
						break;
					...
					case 'single_select_page_with_search':
						...
						if ( ! is_null( $page ) ) {
							...
							$option_display_name = sprintf(
								...
								__( '%1$s (ID: %2$s)', 'woo-variation-gallery' ),
								$page->post_title,
								$option_value
							);
						}
						...
						<tr class="<?php echo esc_attr($class) ?> single_select_page" <?php echo self::dependency_attribute($value); ?>>
							...
							<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
								<select
									...
									data-placeholder="<?php esc_attr_e( 'Search for a page&hellip;', 'woo-variation-gallery' ); ?>"
					...
					case 'single_select_country':
						...
						<tr class="<?php echo esc_attr($class) ?>" <?php echo self::dependency_attribute($value); ?>>
							...
							<td class="forminp"><select name="<?php echo esc_attr( $value['id'] ); ?>" style="<?php echo esc_attr( $value['css'] ); ?>" data-placeholder="<?php esc_attr_e( 'Choose a country / region&hellip;', 'woo-variation-gallery' ); ?>" aria-label="<?php esc_attr_e( 'Country / Region', 'woo-variation-gallery' ); ?>" class="wc-enhanced-select">
					...
					case 'multi_select_countries':
						...
						<tr class="<?php echo esc_attr($class) ?>" <?php echo self::dependency_attribute($value); ?>>
							...
							<td class="forminp">
								<select multiple="multiple" name="<?php echo esc_attr( $value['id'] ); ?>[]" style="width:350px" data-placeholder="<?php esc_attr_e( 'Choose countries / regions&hellip;', 'woo-variation-gallery' ); ?>" aria-label="<?php esc_attr_e( 'Country / Region', 'woo-variation-gallery' ); ?>" class="wc-enhanced-select">
									<?php
									if ( ! empty( $countries ) ) {
									...
									?>
								</select> <?php echo wp_kses_post( $description); ?> <br /><a class="select_all button" href="#"><?php esc_html_e( 'Select all', 'woo-variation-gallery' ); ?></a> <a class="select_none button" href="#"><?php esc_html_e( 'Select none', 'woo-variation-gallery' ); ?></a>
							</td>
						</tr>
						<?php
						break;
					...
					case 'relative_date_selector':
						$periods      = array(
							'days'   => __( 'Day(s)', 'woo-variation-gallery' ),
							'weeks'  => __( 'Week(s)', 'woo-variation-gallery' ),
							'months' => __( 'Month(s)', 'woo-variation-gallery' ),
							'years'  => __( 'Year(s)', 'woo-variation-gallery' ),
						);
